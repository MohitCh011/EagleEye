...
def analyze_image(image_input):
        ...
        try:
            ...
            for class_name, confidence in model_results_1:
                if class_name not in detected_classes_1:
                    detected_classes_1[class_name] = confidence
                else:
                    detected_classes_1[class_name] = max(detected_classes_1[class_name], confidence)
        except Exception as e:
    ...
    # Prepare output in the required format: stage, progress, description
    output = {
        "stage": "analysis complete",  # This could be dynamic depending on progress/phase
        "progress": phase_progress.get("phase 3 completed", 100),  # Use the phase's progress
        "description": {
            "detected_classes": [
                {
                    "class": cls,
                    "confidence": detected_classes_1[cls]
                }
                for cls in detected_classes_1
            ],
